  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <form onSubmit={save} onCopy={e => e.preventDefault()} onCut={e => e.preventDefault()} onPaste={e => e.preventDefault()}>
        ...
        {/* CRT Performance */}
        {form.crtPerformance && form.crtPerformance.length > 0 && (
          <SectionCard icon="🎯" title={`CRT Performance (${form.crtPerformance.length} modules)`} bg="#fef2f2">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {form.crtPerformance.map((p, i) => {
              ...
              {(() => {
                const avg = (form.crtPerformance.reduce((s, p) => s + (p.score/(p.maxScore||100))*100, 0) / form.crtPerformance.length).toFixed(1);
        ...
        {/* Attendance */}
        {form.attendance && form.attendance.length > 0 && (
          <SectionCard icon="📅" title={`Attendance (${form.attendance.length} subjects)`} bg="#ecfeff">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {(() => {
                const avg = (form.attendance.reduce((s, a) => s + (a.present/(a.total||1))*100, 0) / form.attendance.length).toFixed(1);
                const low = form.attendance.filter(a => Math.round((a.present/(a.total||1))*100) < 75).length;
