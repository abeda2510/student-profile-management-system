...
export default function ForgotPassword() {
  ...
  const requestOTP = async (e) => {
    ...
    try {
      ...
      setMaskedEmail(data.maskedEmail); setStep(2);
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
          {['Enter ID', 'Verify OTP', 'New Password'].map((label, i) => (
            <div key={i} style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: step >= i + 1 ? '#1e40af' : '#e2e8f0', color: step >= i + 1 ? '#fff' : '#94a3b8', fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 4px' }}>{i + 1}</div>
          ...
          <form onSubmit={requestOTP}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
              {['student', 'faculty'].map(r => (
                <button key={r} type="button" onClick={() => setUserType(r)}
