...
router.get('/profile-pdf/:regNumber', protect, async (req, res) => {
  try {
    ...
    // Students can only download their own; faculty/admin can download any
    ...
    const row2col = (label1, val1, label2, val2) => {
      ...
      doc.font('Helvetica').fillColor('#0f172a').text(val1 || '—', 145, y, { width: half - 95 });
      if (label2) {
        doc.font('Helvetica-Bold').fillColor(gray).text(label2, 300, y, { width: 90 });
        doc.font('Helvetica').fillColor('#0f172a').text(val2 || '—', 395, y, { width: half - 95 });
      }
      y += 16;
    };
    ...
    const row1col = (label, val) => {
      ...
      doc.font('Helvetica').fillColor('#0f172a').text(val || '—', 175, y, { width: W - 125 });
      y += 16;
    };
    ...
    const semHeaders = ['Sem', 'CGPA', 'SGPA'];
    const colW = [60, 80, 80];
    const tableX = 50;
    // header row
    doc.rect(tableX, y, 220, 16).fill(blue);
    let cx = tableX + 4;
    semHeaders.forEach((h, i) => {
    ...
    for (let s = 1; s <= 8; s++) {
      ...
      cx = tableX + 4;
      const vals = [`Semester ${s}`, cgpa != null ? String(cgpa) : '—', sgpa != null ? String(sgpa) : '—'];
      vals.forEach((v, i) => {
